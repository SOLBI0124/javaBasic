...
public class Inheritance {
	...
	public static void main(String[] args) {
		 ...
		 * 하나의 클래스만 상속받을수있다.
		 * 상속받지 않는 모든 클래스는 object 클래스는 상속받는다.
		 ...
		 * 부모 클래스의 멤버와 자식 클래스의 멤버의 이름이 중복될 때 둘을 구분하기 위해 사용한다.(부모클래스 멤버 앞에super)
		 * 
		 * << super() >>
		 ...
		 * byte	-> int : 값이 온전하게 유지되는가?
		 * 127		20억
		 * 
		 * Parent -> Child  : 멤버를 호출할 수 있는가?
		 * method()  method()
		 * 			 childMethod()
		 * 
		 * 부모와 자식에 동일한 메서드가 있는 경우(오버라이딩 한 경우) 자식의 메서드를 사용한다.
		 * 부모와 자식에 동일한 멤버변수가 있는 경우 부모타입에서는 부모의 변수를 자식타입에서는 자식의 변수를 사용한다.
		 * 
		 * << 접근제어자 >>
		 * 외부로부터 데이터를 보호하거나 감추기 위해 사용한다.
		 * public 	 : 접근에 제한이 없다.
		 * protected : 같은 클래스 + 같은 패키지 + 상속받은 클래스에서 접근이 가능하다.
		 * default 	 : 같은 클래스 + 같은 패키지 				내에서만 접근이 가능하다. 생략.적으면안됨
		...
		//AccessModifier class 참고
		AccessModifier am = new AccessModifier();
		
		System.out.println(am.publicVar);	//public : 어디에서든 접근가능
		am.publicMethod();
		
		
		System.out.println(am.protectedVar);//protected : 같은패키지+상속받은 클래스면 가능
		am.proctedMethod();
		
		
		System.out.println(am.defaultVar);	//default : 같은 패키지내에서 접근가능
		am.defaultMethod();
		
		
//		System.out.println(am.privateVar);	//private : 클래스 내에서만
//		am.privateMethod();
		...
		//Singleton Class
		Singleton s = Singleton.getInstance();	//static이붙은메서드 : class명+.
		
		System.out.println(new Inheritance());
		System.out.println(new Inheritance());	//다른 객체이기 때문에 주소가 다름
		System.out.println(Singleton.getInstance());	
		System.out.println(Singleton.getInstance());	//같은 객체이기 때문에 같은 주소 출력
