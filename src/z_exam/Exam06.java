...
public class Exam06 {
	...
	/*0      [6-1] 다음과 같은 멤버변수를 갖는 SutdaCard클래스를 정의하시오.
			    	타 입       변수명       	설 명
...
/*      [6-2] 문제6-1에서 정의한 SutdaCard클래스에 두 개의 생성자와 info()를 추가해서 실행
    ...
    class SutdaCard {
    (1) 알맞은 코드를 넣어 완성하시오.
    				// 숫자를 문자열로 반환한다. 광(光)인 경우 K를 덧붙인다.
        }
...
/*0      [6-3] 다음과 같은 멤버변수를 갖는 Student클래스를 정의하시오.
			   	타 입    		변수명  	  	설 명
			    String    	name   	 	학생이름
			    int       	ban   	 	반
			    int    		no     	  	번호
...
/*0      [6-4] 문제6-3에서 정의한 Student클래스에 다음과 같이 정의된 두 개의 메서드
...
/*0      [6-5] 다음과 같은 실행결과를 얻도록 Student클래스에 생성자와 info()를 추가하시오.
...
/*0      [6-6] 두 점의 거리를 계산하는 getDistance()를 완성하시오.
...
/*0      [6-7] 문제6-6에서 작성한 클래스메서드 getDistance()를 MyPoint클래스의 인스턴스메서
    드로 정의하시오.
...
/*0      [6-8] 다음의 코드에 정의된 변수들을 종류별로 구분해서 적으시오.
    class PlayingCard {
       int kind;
       int num;
       static int width;
...
/*0      [6-9] 다음은 컴퓨터 게임의 병사(marine)를 클래스로 정의한 것이다. 이 클래스의 멤버
...
/*0      [6-10] 다음 중 생성자에 대한 설명으로 옳지 않은 것은? (모두 고르시오)
...
/*0      [6-11] 다음 중 this에 대한 설명으로 맞지 않은 것은? (모두 고르시오)
...
/*0      [6-12] 다음 중 오버로딩이 성립하기 위한 조건이 아닌 것은? (모두 고르시오) 	답 : c,d
 * 																오버로딩 : 같은 이름의 메서드를 정의하는 것
																파라미터의 개수 또는 타입이 달라야 한다. 파라미터의 개수와 타입으로 구분.
          a. 메서드의 이름이 같아야 한다.
...
/*0      [6-13] 다음 중 아래의 add메서드를 올바르게 오버로딩 한 것은? (모두 고르시오) 		답 : b,c,d
          long add(int a, int b) { return a+b;}							
          a. long add(int x, int y) { return x+y;}						a. 파라미터 타입 같음 ->X
          b. long add(long a, long b) { return a+b;}					
          c. int add(byte a, byte b) { return a+b;}
          d. int add(long a, int b) { return (int)(a+b);}				d. 파라미터 변수 순서다르면 호출될 메서드 구분 가능
          
          */
          
          
/*0      [6-14] 다음 중 초기화에 대한 설명으로 옳지 않은 것은? (모두 고르시오)		//답 : c,e
          a.멤버변수는 자동 초기화되므로 초기화하지 않고도 값을 참조할 수 있다.
          b.지역변수는 사용하기 전에 반드시 초기화해야 한다.
          c.초기화 블럭보다 생성자가 먼저 수행된다.							c.x 초기화블럭이 먼저 수행됨.
          d.명시적 초기화를 제일 우선적으로 고려해야 한다.
          e.클래스변수보다 인스턴스변수가 먼저 초기화된다.						e.x 클래스변수가 먼저초기화됨
          
          */
          
          
/*0      [6-15] 다음중 인스턴스변수의 초기화 순서가 올바른 것은? 				//답 : a? 위에서부터 순서대로
...
/*0      [6-16] 다음 중 지역변수에 대한 설명으로 옳지 않은 것은? (모두 고르시오)		//답 : a, 
          a. 자동 초기화되므로 별도의 초기화가 필요없다.
          b. 지역변수가 선언된 메서드가 종료되면 지역변수도 함께 소멸된다.
          c. 매서드의 매개변수로 선언된 변수도 지역변수이다.0
...
/*0      [6-17] 호출스택이 다음과 같은 상황일 때 옳지 않은 설명은? (모두 고르시오)
          println
          method1
          method2
          main
...
/*0      [6-20] 다음과 같이 정의된 메서드를 작성하고 테스트하시오.
...
/*0      [6-21] Tv클래스를 주어진 로직대로 완성하시오. 완성한 후에 실행해서 주어진 실행결과
    와 일치하는지 확인하라.
