...
/*
0 [7-3] 오버라이딩의 정의와 필요성에 대해서 설명하시오.
*/
	
	
/*
0 [7-4] 다음 중 오버라이딩의 조건으로 옳지 않은 것은? (모두 고르시오)
...
/*	
0	[7-5] 다음의 코드는 컴파일하면 에러가 발생한다. 그 이유를 설명하고 에러를 수정하기
...
/*	
0	[7-6] 자손 클래스의 생성자에서 조상 클래스의 생성자를 호출해야하는 이유는 무엇인
	가?	
*/	
	
	
/*	
0	[7-7] 다음 코드의 실행했을 때 호출되는 생성자의 순서와 실행결과를 적으시오.
	...
	class Child extends Parent {	//Child클래스에 Parent를 상속받음
		int x = 3000;
		Child() {
		this(1000);
		}
...
/*	
0	[7-8] 다음 중 접근제어자를 접근범위가 넓은 것에서 좁은 것의 순으로 바르게 나열한
	것은?
...
/*	
0	[7-9] 다음 중 제어자 final을 붙일 수 있는 대상과 붙였을 때 그 의미를 적은 것이다.
	옳지 않은 것은? (모두 고르시오)
	a. 지역변수 - 값을 변경할 수 없다.
...
/*	
0	[7-10] MyTv2클래스의 멤버변수 isPowerOn, channel, volume을 클래스 외부에서 접근할
...
/*	
0	[7-11] 문제7-10에서 작성한 MyTv2클래스에 이전 채널(previous channel)로 이동하는
...
/*	
0	[7-12] 다음 중 접근 제어자에 대한 설명으로 옳지 않은 것은? (모두 고르시오)
...
/*	
0	[7-15] 클래스가 다음과 같이 정의되어 있을 때, 형변환을 올바르게 하지 않은 것은?
	(모두 고르시오.)
	class Unit {}
	class AirUnit extends Unit {}		//Unit클래스를 AirUnit에 상속
	class GroundUnit extends Unit {}	//Unit클래스를 GroundUnit에 상속
	class Tank extends GroundUnit {}	//GroundUnit을 Tank에 상속
	class AirCraft extends AirUnit {}	//AirUnit을 AirCraft에 상속
...
/*	
0	[7-16] 다음 중 연산결과가 true가 아닌 것은? (모두 고르시오)
	class Car {}
	class FireEngine extends Car implements Movable {}	//car를 fe에 상속함 
														//		fe에 movable인터페이스 상속
	class Ambulance extends Car {}						//car를 ambulance에 상속
...
/*			
0	[7-17] 아래 세 개의 클래스로부터 공통부분을 뽑아서 Unit이라는 클래스를 만들고, 이
...
0	[7-20] 다음의 코드를 실행한 결과를 적으시오.
	...
	class Child extends Parent {			//parent에 상속받은 child
		int x = 200;
		void method() {
